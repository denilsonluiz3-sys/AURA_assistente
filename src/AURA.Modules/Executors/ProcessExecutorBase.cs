...
{
    public ProcessOutputEventArgs(string fileName, string workingDirectory, bool isError, string text, string? correlationId = null)
    {
        FileName = fileName;
...
public abstract class ProcessExecutorBase : IToolExecutor
{
    ...
    protected static async Task<ExecutionResult> RunAsync(string fileName, IEnumerable<string> arguments, ExecutionRequest request, CancellationToken cancellationToken)
    {
        ...
        if (result.ExitCode == 127 && result.StandardError.Contains("shared library", StringComparison.OrdinalIgnoreCase))
        {
            ...
            result = await RunProcessAsync("/data/data/com.termux/files/usr/bin/bash", BuildShellCommand(fileName, arguments), fallbackRequest, cancellationToken);
        }
        return result;
    }
    ...
    private static void ApplyTermuxEnvironment(ProcessStartInfo psi, string fileName)
    {
        ...
        if (!OperatingSystem.IsAndroid() || !fileName.StartsWith(termuxRoot, StringComparison.Ordinal)) return;
        ...
        var currentPath = psi.Environment.TryGetValue("PATH", out var existingPath) ? existingPath : Environment.GetEnvironmentVariable("PATH") ?? string.Empty;
        psi.Environment["PATH"] = binDir + Path.PathSeparator + currentPath;
        if (Directory.Exists(libDir)) psi.Environment["LD_LIBRARY_PATH"] = libDir;
        if (!psi.Environment.ContainsKey("HOME") && Directory.Exists(home)) psi.Environment["HOME"] = home;
    }
    ...
    private static async Task<ExecutionResult> RunProcessAsync(string fileName, IEnumerable<string> arguments, ExecutionRequest request, CancellationToken cancellationToken)
    {
        ...
        if (request.EnvironmentVariables is not null)
            foreach (var (key, value) in request.EnvironmentVariables) psi.Environment[key] = value;
        ...
        Task ReadStreamAsync(StreamReader reader, StringBuilder buffer, bool isError) => Task.Run(async () =>
        {
            while (true)
            {
                ...
                OutputReceived?.Invoke(null, new ProcessOutputEventArgs(fileName, workingDirectory, isError, line + Environment.NewLine, correlationId));
            }
        });
        ...
        using var cts = request.Timeout is not null ? CancellationTokenSource.CreateLinkedTokenSource(cancellationToken) : null;
    ...
    protected static string? ResolveBinary(params string[] candidates)
    {
        ...
        {
            string[] termuxDirs = { "/data/data/com.termux/files/usr/bin", "/data/data/com.termux/files/usr/local/bin", "/data/data/com.termux/files/home/.local/bin" };
            foreach (var termuxDir in termuxDirs) if (!dirs.Contains(termuxDir)) dirs.Add(termuxDir);
        }
