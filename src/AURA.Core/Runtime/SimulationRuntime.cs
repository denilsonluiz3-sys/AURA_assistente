...
namespace AURA.Core.Runtime
{
    ...
    public sealed class SimulationRuntime : IDisposable
    {
        ...
        private readonly HashSet<string> _transientCellIds = new HashSet<string>(StringComparer.OrdinalIgnoreCase);
        ...
        public IReadOnlyCollection<Cell> Cells => _cells.Values.Where(c => !c.IsTransient).ToArray();
        ...
        /// <summary>Creates a cell only for the current session. It is never indexed, logged, or restored.</summary>
        public Cell CreateTransientCell(string id, string appPath, string? args = null)
        {
            if (string.IsNullOrWhiteSpace(id)) throw new ArgumentException("O id da célula não pode ser vazio.", nameof(id));
            if (string.IsNullOrWhiteSpace(appPath)) throw new ArgumentException("O caminho do aplicativo não pode ser vazio.", nameof(appPath));
            if (_cells.ContainsKey(id)) throw new InvalidOperationException("Já existe uma célula com id '" + id + "'.");

            var cell = new Cell
            {
                Id = id, AppPath = appPath, Args = args ?? string.Empty,
                WorkingDirectory = Path.GetDirectoryName(appPath),
                CellRoot = Path.Combine(_cellsRoot, id), IsTransient = true
            };
            _backend.Create(cell);
            _cells[id] = cell;
            _transientCellIds.Add(id);
            return cell;
        }

        /// <summary>Stops and removes a session-only cell, including temporary logs and state.</summary>
        public void DeleteTransientCell(string id)
        {
            if (!_transientCellIds.Contains(id)) return;
            if (_cells.TryGetValue(id, out Cell? cell))
            {
                if (cell.State == CellState.Running || cell.State == CellState.Paused) StopCell(id);
                CloseLogWriter(id);
                try { _backend.Delete(cell); } catch { }
                _cells.TryRemove(id, out _);
            }
            _transientCellIds.Remove(id);
        }
        ...
        public string ReadCellLog(string id, int tailLines = 50)
        {
            ...
            return string.Join("\n", File.ReadLines(cell.LogFile).Reverse().Take(tailLines).Reverse());
        }
        ...
        public void Dispose()
        {
            ...
            foreach (string id in _transientCellIds.ToArray())
            {
                try { DeleteTransientCell(id); } catch { }
            }

            _cells.Clear();
            _transientCellIds.Clear();
            _processes.Clear();
