    ...
    public static class AuraLog
    {
        ...
        public static void Init(Context context)
        {
            try
            {
                lock (Sync)
                {
                    ...
                    string baseDir = context.GetExternalFilesDir(null)?.AbsolutePath ?? context.FilesDir?.AbsolutePath;
                    if (!string.IsNullOrEmpty(baseDir))
                    {
                        string logsDir = Path.Combine(baseDir, "logs");
                        Directory.CreateDirectory(logsDir);
                        _filePath = Path.Combine(logsDir, string.Format("aura_{0:yyyyMMdd_HHmmss}.log", DateTime.Now));
                        _fileReady = true;
        ...
        private static void TryCreateDownloadMirror(Context context)
        {
            try
            {
                ...
                Android.Net.Uri? uri = context.ContentResolver.Insert(MediaStore.Downloads.ExternalContentUri, values);
                if (uri == null) return;
        ...
        public static void WireGlobalExceptionHandlers()
        {
            try
            {
                ...
                {
                    if (args.ExceptionObject is Exception ex) Exception("AppDomain.UnhandledException", ex);
        ...
        public static string LogFilePath { get { lock (Sync) return _filePath; } }
        ...
        public static string ReadRecentLog(int maxLines = 500)
        {
            try
            {
                lock (Sync)
                {
                    if (!_fileReady || string.IsNullOrEmpty(_filePath) || !File.Exists(_filePath)) return PendingBuffer.ToString();
                }
                ...
                for (int i = lines.Length - maxLines; i < lines.Length; i++) sb.AppendLine(lines[i]);
                return sb.ToString();
            }
            catch { return "(falha ao ler o log)"; }
        }
        ...
        public static string Redact(string? message)
        {
            ...
            try { return SecretPatterns.Replace(message, "[REDACTED]"); }
        ...
        private static void Write(string level, string message)
        {
            ...
            string line = string.Format("{0:yyyy-MM-dd HH:mm:ss.fff} [{1}] {2}", DateTime.Now, level, safe);
            lock (Sync)
            {
                if (_fileReady)
                {
                    try { File.AppendAllText(_filePath, line + Environment.NewLine); } catch { }
                    try { _downloadWriter?.WriteLine(line); } catch { }
                }
                else
                {
                    PendingBuffer.AppendLine(line);
                    if (PendingBuffer.Length > 32768) PendingBuffer.Remove(0, 16384);
                }
            }
            try
            {
                if (level == "ERROR" || level == "FATAL" || level == "EXCPT") Log.Error(LogcatTag, line);
                else if (level == "WARN ") Log.Warn(LogcatTag, line);
                else Log.Info(LogcatTag, line);
            }
            catch { }
        ...
        private sealed class AuraUncaughtExceptionHandler : Java.Lang.Object, Java.Lang.Thread.IUncaughtExceptionHandler
        {
            ...
            public AuraUncaughtExceptionHandler(Java.Lang.Thread.IUncaughtExceptionHandler? next) { _next = next; }
            public void UncaughtException(Java.Lang.Thread? thread, Java.Lang.Throwable? throwable)
            {
                ...
                {
                    try { _next.UncaughtException(thread, throwable); } catch { }
                }
            }
        }
    }
