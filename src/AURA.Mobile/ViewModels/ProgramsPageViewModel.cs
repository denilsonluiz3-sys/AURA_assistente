...
namespace AURA.Mobile.ViewModels
{
    public sealed class ProgramsPageViewModel : INotifyPropertyChanged
    {
        ...
        public ProgramsPageViewModel(CellProgramRegistry registry, CellProgramRunner runner, IAuraCellContextFactory contextFactory, ILogger logger)
        {
            _registry = registry;
            _runner = runner;
    ...
    public sealed class ProgramCardViewModel : INotifyPropertyChanged
    {
        ...
        public ProgramCardViewModel(IAuraCellProgram program, CellProgramRunner runner, IAuraCellContextFactory contextFactory, ILogger logger)
        {
            _program = program;
            _runner = runner;
            _contextFactory = contextFactory;
            _logger = logger;
            ExecuteCommand = new Command(async () => await ExecuteAsync(), () => !_isRunning);
        }
        ...
        public ProgramState State { get => _state; private set { _state = value; OnPropertyChanged(); OnPropertyChanged(nameof(StatusText)); OnPropertyChanged(nameof(StatusColor)); OnPropertyChanged(nameof(ActionButtonText)); OnPropertyChanged(nameof(CanExecute)); } }
        public string? LastResult { get => _lastResult; private set { _lastResult = value; OnPropertyChanged(); OnPropertyChanged(nameof(HasResult)); OnPropertyChanged(nameof(ResultSummary)); } }
        ...
        private async Task ExecuteAsync()
        {
            ...
            try
            {
                var cellId = $"program-ui-{Guid.NewGuid():N}";
                var context = _contextFactory.Create(cellId, cts.Token);
                var result = await _runner.RunAsync(_program, context, cts.Token);
                if (result.IsSuccess) { State = ProgramState.Success; LastResult = FormatResult(result.Data); }
                else { State = ProgramState.Failed; LastResult = $"Erro: {result.Error}"; }
            }
            ...
            finally { _isRunning = false; OnPropertyChanged(nameof(CanExecute)); OnPropertyChanged(nameof(ActionButtonText)); }
        }
