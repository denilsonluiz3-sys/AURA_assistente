...
/// <summary>Configuração persistente escolhida pelo usuário. Não contém providers/modelos pré-configurados.</summary>
public static class RuntimeConfig
{
    ...
    {
        var id = Normalize(providerId); if (string.IsNullOrEmpty(id)) return string.Empty;
        try { return SecureStorage.Default.GetAsync(ApiKeyPrefix + id).GetAwaiter().GetResult()?.Trim() ?? string.Empty; } catch { return string.Empty; }
    }
    public static void SetApiKeyForProvider(string? providerId, string? value)
    {
        var id = Normalize(providerId); if (string.IsNullOrEmpty(id)) return;
        try { var key = ApiKeyPrefix + id; var v = (value ?? string.Empty).Trim(); if (v.Length == 0) SecureStorage.Default.Remove(key); else SecureStorage.Default.SetAsync(key, v).GetAwaiter().GetResult(); } catch { }
    }
    ...
    {
        var provider = Provider.Trim(); var model = Model.Trim(); var baseUrl = NormalizeChatBaseUrl(BaseUrlOverride);
        if (string.IsNullOrWhiteSpace(provider)) throw new InvalidOperationException("Provider não configurado.");
        if (string.IsNullOrWhiteSpace(model)) throw new InvalidOperationException("Modelo não configurado.");
        if (string.IsNullOrWhiteSpace(baseUrl)) throw new InvalidOperationException("Endpoint não configurado.");
        return UniversalRuntimeAdapter.CreateConnection(provider, GetApiKeyForProvider(provider), model, baseUrl, ModelsUrlOverride, ApiFormat, AuthHeader, AuthScheme, RequiresApiKey);
    }

    public static void Apply(IUniversalAiClient client)
    {
        ArgumentNullException.ThrowIfNull(client);
        var c = CreateConnection(); var configured = UniversalAiClientFactory.Create(c, MaxTokens, TimeoutSeconds);
        client.Options.Provider = configured.Options.Provider; client.Options.ApiKey = configured.Options.ApiKey; client.Options.BaseUrl = configured.Options.BaseUrl; client.Options.Model = configured.Options.Model;
        client.Options.MaxTokens = configured.Options.MaxTokens; client.Options.TimeoutSeconds = configured.Options.TimeoutSeconds; client.Options.AuthHeaderName = configured.Options.AuthHeaderName; client.Options.AuthScheme = configured.Options.AuthScheme; client.Options.ApiFormat = configured.Options.ApiFormat;
