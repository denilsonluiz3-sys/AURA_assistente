...
public static class RuntimeConfig
{
    ...
    {
        get => Enum.TryParse<UniversalApiFormat>(Preferences.Default.Get("ai_api_format", string.Empty), true, out var f) ? f : UniversalApiFormat.OpenAiCompatible;
    ...
    public static void SetApiKeyForProvider(string? providerId, string? value)
    {
        ...
        try
        {
            var key = ApiKeyPrefix + id; var v = (value ?? string.Empty).Trim();
            if (v.Length == 0) SecureStorage.Default.Remove(key); else SecureStorage.Default.SetAsync(key, v).GetAwaiter().GetResult();
        }
        catch { }
    }
    ...
    private static string Normalize(string? value)
    {
        ...
        return s.Length == 0 ? string.Empty : new string(s.Select(c => char.IsLetterOrDigit(c) || c is '-' or '_' ? c : '_').ToArray());
    }
}
