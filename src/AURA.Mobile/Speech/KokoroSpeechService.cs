...
namespace AURA.Mobile.Speech
{
    ...
    public sealed class KokoroSpeechService : ISpeechService, IDisposable
    {
        ...
        public async Task InitializeAsync(CancellationToken ct = default)
        {
            ...
            try
            {
                modelPath = await CopyAssetToCacheAsync("kokoro-v1.0.int8.onnx", ct).ConfigureAwait(false);
            }
            catch (FileNotFoundException)
            {
                // Modelo kokoro ausente do pacote: motor indisponível.
                throw new NotSupportedException(
                    "TTS Kokoro indisponível: modelo kokoro-v1.0.int8.onnx não está no pacote.");
            }
            ...
            try
            {
                voice = await LoadVoiceAsync(ct).ConfigureAwait(false);
            }
            catch (FileNotFoundException)
            {
                throw new NotSupportedException(
                    "TTS Kokoro indisponível: pf_dora.f32 não está no pacote.");
            }
            ct.ThrowIfCancellationRequested();
