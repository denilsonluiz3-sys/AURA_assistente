    ...
    public partial class BrowserPage : ContentPage
    {
        ...
        private void StartIsolatedSession()
        {
            ...
            try
            {
                ...
                _runtime.CreateTransientCell(id, "com.aura.webview", "browser-isolado");
                _isolatedCellId = id;
                _isolated = true;
        ...
        private void EndIsolatedSession()
        {
            ...
            try
            {
                ...
                _isolatedCellId = null;
                IsolatedLabel.IsVisible = false;
                _runtime.DeleteTransientCell(id);
                PurgeBrowsingData();
            }
            catch (Exception ex)
            {
        ...
        private async void HandleOnion(string input)
        {
            ...
            string message = installed
                ? "Endereço .onion exige uma conexão privada ativa.\n\nAbra o Orbot e ative o modo VPN (a conexão de todo o aparelho passa pelo provedor — então o WebView consegue acessar .onion). Depois toque em 'Abrir Orbot' e tente de novo."
                : "Endereço .onion exige uma conexão privada, que não vem no Android.\n\nInstale um provedor compatível: ele oferece modo VPN que roteia o app inteiro pela rede privada, permitindo abrir .onion no navegador. O WebView da AURA não pode se conectar a .onion sem ele.";

            string action = installed ? "Abrir Orbot" : "Instalar Orbot";

            bool? answer = await DisplayAlertAsync("Acesso privado (.onion)", message, action, "Cancelar");
            if (answer == true)
            {
#if ANDROID
                if (installed)
